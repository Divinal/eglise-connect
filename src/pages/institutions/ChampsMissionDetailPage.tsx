  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      const { data: ann } = await (supabase as any).from("announcements").select("*")
        .eq("entity_type", "champs_mission").eq("entity_id", uuid).eq("status", "approved")
  ...
  if (!champ) return (
    <Layout>
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <p className="text-muted-foreground">Champ de mission introuvable.</p>
        <Button variant="outline" onClick={() => navigate("/institution/champs-de-mission")}>
          <ArrowLeft className="h-4 w-4 mr-2" /> Retour aux Champs de Mission
        </Button>
      </div>
    </Layout>
  );
    ...
    <Layout>
      <div className="bg-primary py-10">
        <div className="container">
          <button onClick={() => navigate("/institution/champs-de-mission")}
            ...
            <ArrowLeft className="h-4 w-4" /> Tous les Champs de Mission
          </button>
            ...
            <main className="flex-1 min-w-0">
              ...
              {annonces.length === 0 ? (
                <div className="bg-white rounded-xl border border-border p-10 text-center">
                  <p className="text-muted-foreground text-sm">Aucune annonce pour ce champ de mission.</p>
                </div>
              ) : (
...
export default ChampsMissionDetailPage;

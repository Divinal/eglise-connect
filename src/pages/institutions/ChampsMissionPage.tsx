...
const ChampsMissionPage = () => {
  const navigate = useNavigate();
  const [items, setItems] = useState<ChampMission[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    (supabase as any).from("champs_mission")
    ...
    <Layout>
      <div className="bg-primary py-12">
        <div className="container">
          <h1 className="font-display text-3xl md:text-4xl text-primary-foreground">Champs de Mission</h1>
          <p className="text-primary-foreground/70 mt-2">Zones missionnaires de l'Église Évangélique du Congo</p>
        </div>
      </div>
        ...
        <div className="container">
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-muted-foreground">Aucun champ de mission pour l'instant.</p>
            </div>
          ) : (
...
export default ChampsMissionPage;

...
const CTPADPage = () => {
  ...
  useEffect(() => {
    supabase.from("announcements").select("*").eq("entity_type", "ctpad").eq("status", "approved")
      .order("created_at", { ascending: false }).then(({ data }) => setAnnonces(data || []));
    (supabase.from("membres") as any).select("*").eq("entity_type", "ctpad").order("nom")
    ...
    <Layout>
      <div className="bg-primary py-12">
        <div className="container">
          <h1 className="font-display text-3xl md:text-4xl text-primary-foreground">CTPAD</h1>
          <p className="text-primary-foreground/70 mt-2 max-w-2xl">
            Coordination Technique de Projet et d'Appuis au Développement — l'ONG de l'Église
            Évangélique du Congo en charge des projets de développement.
          </p>
        </div>
      </div>
              ...
              <aside className="w-full lg:w-72 shrink-0">
                <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
                  <div className="bg-[#2e7d32] px-4 py-3">
                    <h3 className="font-semibold text-white text-sm">Équipe CTPAD</h3>
                  </div>

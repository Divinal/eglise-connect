...
import { Heart, Users, MapPin, Phone, Mail, ArrowLeft, ChevronDown } from "lucide-react";
...
const ChampsEvangelisationDetailPage = () => {
  const { slug } = useParams<{ slug: string }>();
  const navigate = useNavigate();
  const [champ, setChamp] = useState<ChampEvang | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      setLoading(true);
      const { data: allItems } = await (supabase as any).from("champs_evangelisation").select("*");
  ...
  if (!champ) return (
    <Layout>
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <p className="text-muted-foreground">Champ d'évangélisation introuvable.</p>
        <Button variant="outline" onClick={() => navigate("/institution/champs-evangelisation")}>
          <ArrowLeft className="h-4 w-4 mr-2" /> Retour aux Champs d'Évangélisation
        </Button>
      </div>
    </Layout>
  );
    ...
    <Layout>
      <div className="bg-primary py-10">
        <div className="container">
          <button onClick={() => navigate("/institution/champs-evangelisation")}
            ...
            <ArrowLeft className="h-4 w-4" /> Tous les Champs d'Évangélisation
          </button>
          <h1 className="font-display text-3xl md:text-4xl text-primary-foreground">{champ.name}</h1>
          {champ.description && (
            <p className="text-primary-foreground/70 mt-2 max-w-2xl text-sm">{champ.description}</p>
          )}
          <div className="flex flex-wrap gap-4 mt-4">
            {champ.ville && <div className="flex items-center gap-1.5 text-primary-foreground/70 text-sm"><MapPin className="h-3.5 w-3.5" />{champ.ville}</div>}
            {champ.telephone && <div className="flex items-center gap-1.5 text-primary-foreground/70 text-sm"><Phone className="h-3.5 w-3.5" />{champ.telephone}</div>}
            {champ.email && <div className="flex items-center gap-1.5 text-primary-foreground/70 text-sm"><Mail className="h-3.5 w-3.5" />{champ.email}</div>}
            {champ.responsable && <div className="flex items-center gap-1.5 text-primary-foreground/70 text-sm"><Users className="h-3.5 w-3.5" />Responsable : {champ.responsable}</div>}
          </div>
        </div>
      </div>
      ...
      {champ.historique && (
        <section className="py-10 bg-white border-b border-border">
          <div className="container max-w-3xl">
            ...
            <p className="text-muted-foreground leading-relaxed text-base whitespace-pre-wrap">{champ.historique}</p>
          </div>
        </section>
      )}
            ...
            <main className="flex-1 min-w-0">
              ...
              {annonces.length === 0 ? (
                <div className="bg-white rounded-xl border border-border p-10 text-center">
                  <p className="text-muted-foreground text-sm">Aucune annonce pour ce champ d'évangélisation.</p>
                </div>
              ) : (
...
export default ChampsEvangelisationDetailPage;

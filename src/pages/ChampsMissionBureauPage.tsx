...
const ChampsMissionBureauPage = () => {
  ...
  useEffect(() => {
    if (!loading && !isAdminGeneral && !hasRole("admin_champs_mission", id)) navigate("/admin");
  }, [loading, isAdminGeneral, id]);

  useEffect(() => {
    (supabase as any).from("champs_mission").select("name").eq("id", id!).maybeSingle()
      .then(({ data }: any) => setNomEntite(data?.name || "Champ de Mission"));
  }, [id]);
    ...
    <Layout>
      <div className="bg-primary py-8">
        <div className="container flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={() => navigate(isAdminGeneral ? "/admin/champs-mission" : "/admin")}
      ...
      <section className="py-8 bg-muted/30 min-h-[60vh]">
        <div className="container max-w-4xl">
          ...
          {activeTab === "bureau" && id && (
            <MembresManager entityType="champs_mission" entityId={id} memberType="bureau" title="Bureau" />
          )}
          {activeTab === "conseil" && id && (
            <MembresManager entityType="champs_mission" entityId={id} memberType="conseil" title="Conseil" />
          )}
          {activeTab === "commissions" && id && (
            <MembresManager entityType="champs_mission" entityId={id} memberType="organe" title="Commissions" />
          )}
          {activeTab === "organes" && id && (
            <MembresManager entityType="champs_mission" entityId={id} memberType="organe" title="Organes" />
          )}
...
export default ChampsMissionBureauPage;

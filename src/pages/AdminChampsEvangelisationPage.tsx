...
import { Heart, ArrowLeft, Plus, Pencil, Save, X, ChevronRight } from "lucide-react";
...
const AdminChampsEvangelisationPage = () => {
  ...
  const fetchItems = async () => {
    setFetching(true);
    const { data, error } = await (supabase as any).from("champs_evangelisation").select("*").order("name");
  ...
  const handleSave = async () => {
    ...
    if (editingId) {
      const { error } = await (supabase as any).from("champs_evangelisation").update(payload).eq("id", editingId);
      if (error) { toast({ title: "Erreur", description: error.message, variant: "destructive" }); return; }
      toast({ title: "Champ d'évangélisation mis à jour" });
    } else {
      const { error } = await (supabase as any).from("champs_evangelisation").insert(payload);
      if (error) { toast({ title: "Erreur", description: error.message, variant: "destructive" }); return; }
      toast({ title: "Champ d'évangélisation créé !" });
    }
    setForm(emptyForm); setShowForm(false); setEditingId(null);
    fetchItems();
  };

  const startEdit = (c: ChampEvang) => {
    setEditingId(c.id);
  ...
  const toggleActive = async (c: ChampEvang) => {
    await (supabase as any).from("champs_evangelisation").update({ is_active: !c.is_active }).eq("id", c.id);
    fetchItems();
  };
      ...
      <div className="bg-primary py-8">
        <div className="container flex items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="font-display text-2xl text-primary-foreground">Champs d'Évangélisation</h1>
            <p className="text-primary-foreground/70 text-sm mt-0.5">Gérer les champs d'évangélisation de l'EEC</p>
          </div>
        ...
        <div className="container max-w-4xl">
          {showForm && (
            <div className="bg-card border border-border rounded-xl p-6 mb-6 shadow-sm">
              <h2 className="font-semibold text-foreground mb-4">
                {editingId ? "Modifier le champ d'évangélisation" : "Nouveau champ d'évangélisation"}
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <Label>Nom *</Label>
                  <Input value={form.name} onChange={e => setForm({ ...form, name: e.target.value, slug: toSlug(e.target.value) })} placeholder="Ex: Zone Nord-Congo" />
                </div>
                <div className="space-y-1.5">
                  <Label>Slug (auto-généré)</Label>
                  <Input value={form.slug} onChange={e => setForm({ ...form, slug: e.target.value })} placeholder="zone-nord-congo" />
                </div>
                <div className="space-y-1.5">
                  <Label>Responsable</Label>
                  <Input value={form.responsable} onChange={e => setForm({ ...form, responsable: e.target.value })} placeholder="Nom de l'évangéliste responsable" />
                </div>
                <div className="space-y-1.5">
                  <Label>Zone / Localité</Label>
                  <Input value={form.ville} onChange={e => setForm({ ...form, ville: e.target.value })} placeholder="Owando, Cuvette" />
                </div>
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  <Label>Email</Label>
                  <Input value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder="evang@eec.cg" />
                </div>
          ...
          {fetching ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-muted-foreground">Aucun champ d'évangélisation. Créez-en un.</p>
            </div>
          ) : (
              ...
              {items.map(c => (
                <div key={c.id} className={`bg-card border rounded-xl p-5 flex items-center gap-4 ${c.is_active ? "border-border" : "border-border/50 opacity-60"}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {c.ville && <p className="text-xs text-muted-foreground">Zone : {c.ville}</p>}
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <Button variant="ghost" size="icon" onClick={() => navigate(`/admin/champs-evangelisation/${c.id}/bureau`)} className="h-8 w-8">
...
export default AdminChampsEvangelisationPage;
